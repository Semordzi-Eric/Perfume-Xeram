...
const passwordMismatch = computed(() =>
  !!(state.value.password && state.value.confirmPassword && state.value.password !== state.value.confirmPassword),
)
...
const strengthLabel = computed(() => {
  return ['', 'Weak', 'Fair', 'Strong', 'Excellent'][passwordStrength.value]
})
...
<template>
  <section class="auth-page min-h-screen grid grid-cols-1 lg:grid-cols-2">
    ...
    <!-- LEFT — Form (on signup, form is on the left so it differs from Login) -->
    <div class="flex flex-col justify-center px-8 sm:px-12 lg:px-20 py-16 lg:py-24 bg-ivory dark:bg-obsidian relative overflow-hidden">
      ...
      <div class="lg:hidden text-center mb-12 pt-4">
        <p class="eyebrow mb-3">Join the Maison</p>
        <h2 class="font-editorial text-4xl text-obsidian dark:text-ivory mb-3">
          Create <span class="italic font-display text-gold">your account.</span>
        </h2>
        <div class="gold-divider mt-3" />
      </div>

      <!-- Background numeral -->
      <span class="absolute top-1/2 -left-20 -translate-y-1/2 numeral text-[24rem] text-gold/[0.04] dark:text-gold/[0.06] pointer-events-none select-none leading-none">
        ✧
      </span>
      ...
      <div class="relative z-10 w-full max-w-md mx-auto">
        ...
        <div class="hidden lg:block mb-10">
          <p class="eyebrow mb-4">Create Account</p>
        ...
        <form @submit="handleSignup">
          ...
          <div class="form-field">
            <label class="form-label flex items-center justify-between">
              <span>Password</span>
              <span v-if="passwordStrength" class="text-[9px] tracking-[0.25em] uppercase font-light italic font-display normal-case" :class="`pw-${passwordStrength}`">
                {{ strengthLabel }}
              </span>
            </label>
            <div class="relative">
            ...
            <!-- Strength meter -->
            <div v-if="state.password" class="grid grid-cols-4 gap-1 mt-2">
              <div
                v-for="i in 4"
                :key="i"
                class="h-px transition-colors duration-400"
                :class="i <= passwordStrength ? `pw-bar-${passwordStrength}` : 'bg-gold/15'"
              />
            </div>
          </div>
          ...
          <div class="form-field">
            ...
            <p v-if="passwordMismatch" class="mt-2 text-[9px] tracking-[0.25em] uppercase text-red-400/80 font-light italic">
              Passwords do not match
            </p>
          </div>

          <!-- Terms -->
          <label class="flex items-start gap-3 cursor-none mt-3">
            <span class="custom-check mt-0.5" :class="state.agreeToTerms ? 'is-checked' : ''">
              <span class="check-mark">✓</span>
              <input v-model="state.agreeToTerms" type="checkbox" class="sr-only" />
            </span>
            <span class="text-[10px] tracking-[0.25em] uppercase text-ash font-light leading-relaxed">
              I agree to the
              <a href="#" class="text-gold hover-line">Terms</a> &
              <a href="#" class="text-gold hover-line">Privacy Policy</a>
            </span>
          </label>
        ...
        <!-- Login CTA -->
        <p class="text-center mt-10 text-[10px] tracking-[0.3em] uppercase text-ash font-light">
          Already have an account?
          <router-link :to="{ name: 'login' }" class="ml-2 text-gold hover-line">Sign In</router-link>
        </p>
      </div>
    </div>

    <!-- RIGHT — Atmospheric image panel -->
    <div class="hidden lg:flex relative overflow-hidden bg-obsidian order-first lg:order-last">
      <img
        src="/img/discover.png"
        alt="Xeram"
        class="absolute inset-0 w-full h-full object-cover opacity-55 transition-transform duration-[8s] hover:scale-105"
      />
      ...
      <!-- Corner ornaments -->
      <div class="absolute top-10 right-10 corner-tr" />
      <div class="absolute bottom-10 left-10 corner-bl" />

      <!-- Vertical text -->
      <div class="absolute right-10 top-1/2 -translate-y-1/2 flex flex-col items-center gap-6">
      ...
      <div class="relative z-10 flex flex-col justify-end p-16 w-full">
        <p class="eyebrow mb-5">Join the Maison</p>
        <h2 class="font-editorial text-5xl xl:text-7xl text-ivory leading-[0.92] tracking-tight mb-6">
          A house <span class="italic font-display text-gold-shimmer">opens</span> its doors.
        </h2>
        ...
        <p class="font-display italic text-base xl:text-lg text-ivory/75 leading-snug max-w-md">
          Save your favorites. Track your orders. Hear about new compositions before they reach the shop floor.
        </p>

        <!-- Perks list -->
        <ul class="mt-10 space-y-3">
          <li class="flex items-center gap-3 text-[10px] tracking-[0.3em] uppercase text-ivory/60 font-light">
...
/* Corners */
.corner-tr { width: 32px; height: 32px; border-top: 1px solid rgba(232,213,163,0.5); border-right: 1px solid rgba(232,213,163,0.5); }
.corner-bl { width: 32px; height: 32px; border-bottom: 1px solid rgba(232,213,163,0.5); border-left: 1px solid rgba(232,213,163,0.5); }
...
.pw-3 { color: #b6a44b; }
.pw-4 { color: #c9a84c; }
.pw-bar-1 { background: #d97757; }
.pw-bar-2 { background: #d4a13c; }
.pw-bar-3 { background: linear-gradient(90deg, #b6a44b, #c9a84c); }
.pw-bar-4 { background: linear-gradient(90deg, #c9a84c, #e8d5a3); }
